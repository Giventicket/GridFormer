...
class TSPImageDataset(torch.utils.data.Dataset):
    ...
    def get_reordered_tsp_instance(self, points, tour):
        ...
        before_indexed_reordered_tour = []
        for after_idx in reordered_tour:
            before_indexed_reordered_tour.append(after_idx2before_idx[after_idx])
        before_indexed_reordered_tour = np.array(before_indexed_reordered_tour)

        return points, before_indexed_reordered_tour, center_of_mass
    ...
    def rasterize(self, idx):
        ...
        cv2.circle(
            img,
            ((self.img_size - 1) * center_of_mass).astype(int),
...
if __name__ == "__main__":
    ...
    with open(f"reordered(tour_only)_{data_file}", "w") as file:
