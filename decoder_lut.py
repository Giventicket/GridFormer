...
def get_embedding3(dist_matrix, angle_matrix, device="cuda"):
    num_sectors = 12
...
def get_decoder_embedding(tsp_instance, visited):
    ...
    visited_embedding2 = get_embedding2(dist_matrix, device)
    visited_embedding3 = get_embedding3(dist_matrix, angle_matrix, device)

    unvisited_embedding2 = get_embedding2(unvisited_dist_matrix, device)
    unvisited_embedding3 = get_embedding3(unvisited_dist_matrix, angle_matrix, device)

    embedding = torch.concat(
        [embedding1, visited_embedding2, visited_embedding3, unvisited_embedding2, unvisited_embedding3], dim=-1
    ).type(torch.float)
    return embedding
